...
class AllTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
                    ...
                    <div class="dropdown">
                        <div class="btn-group mb-0">
                            ...
                            <div class="dropdown-content">
                                <a href="' . route('admin.all-tasks.edit', [$row->id]) . '"><img src="'.asset('img/icons/edit.svg').'" /> ' . trans('app.edit') . '</a>';                               
                            ...
                            $action .= '<a href="javascript:;" class="sa-params"
                                  data-task-id="' . $row->id . '" data-recurring="' . $recurringTask . '"><img src="'.asset('img/icons/delete.svg').'" /> ' . trans('app.delete') . '</a>';
                            '</div>
                        </div>
                    </div>
                </div>';
            ...
            ->addColumn('checkbox', function ($row) {
                return '<input type="checkbox" class="sub_chk" value="'.$row->id.'">';
            })
    ...
    public function query(Task $model)
    {
        ...
        $inCompleteColumn = TaskboardColumn::where('slug', 'incomplete')->first();
        ...
        if ($request->status_by == 'incomplete') {
            $model = $model->where('tasks.board_column_id', $inCompleteColumn->id);
        }

        if ($request->status_by == 'completed') {
            $model = $model->where('tasks.board_column_id', $taskBoardColumn->id);
        }

        if ($request->status_by == 'highPriority') {
            $model = $model->where('tasks.priority', 'high');
        }

        if ($request->status_by == 'dueDate') {
            $model = $model->where('tasks.due_date', '<', Carbon::now())->where('tasks.board_column_id', $inCompleteColumn->id);
        }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->buttons(Button::make([
                ...
                'buttons' => [
                                ['text' => 'Incomplete', 'className' => 'btn-action', 'attr' => ['data-status' => 'incomplete'] ],
                                ['text' => 'Complete', 'className' => 'btn-action', 'attr' => ['data-status' => 'complete'] ],
                                ['text' => 'Complete & Archive', 'className' => 'btn-archive', 'attr' => ['data-status' => 'archive'] ],
                                ['text' => 'Delete', 'className' => 'btn-delete', 'attr' => ['data-status' => 'delete']],
                            ]
                //'fade' => true
                ]))
            ->parameters([
    ...
    protected function getColumns()
    {
        return [
            ...
            __('<input title="select" type="checkbox" class="master">') => ['data' => 'checkbox', 'name' => 'checkbox', 'orderable' => false, 'searchable' => false],
