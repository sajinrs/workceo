...
class ClientsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
                    ...
                    <div class="dropdown">
                        <div class="btn-group mb-0">
                            ...
                            <div class="dropdown-content">
                              <a href="' . route('admin.clients.edit', [$row->id]) . '"><img src="'.asset('img/icons/edit.svg').'" /> ' . trans('app.edit') . '</a>
                              <a href="' . route('admin.clients.projects', [$row->user_id]) . '"><img src="'.asset('img/icons/search.svg').'" /> ' . __('app.view') . '</a>
                              <a href="javascript:;"  data-user-id="' . $row->user_id . '"  class="sa-params"><img src="'.asset('img/icons/delete.svg').'" /> ' . trans('app.delete') . '</a>
            ...
            ->editColumn(
                'name',
                function ($row) {
                    return '<a href="' . route('admin.contacts.show', $row->user_id) . '">' . ucfirst($row->name) . '</a>';
                }
            )
            ->editColumn(
                'created_at',
            ...
            ->editColumn(
                ...
                function ($row) {
                    if($row->status == 'active'){
                        ...
                        return '<label class="badge badge-danger w-100">'.__('app.inactive').'</label>';
                    }
                }
            )
            ->addIndexColumn()
            ->rawColumns(['name', 'action','Phone Number', 'status']);
    }
    ...
    public function query(ClientDetails $model)
    {
        ...
        $model = $model->join('users', 'client_details.user_id', '=', 'users.id')
            ->select('client_details.id', 'client_details.user_id', 'client_details.name', 'client_details.company_name', 'client_details.email','client_details.mobile', 'client_details.created_at', 'users.status')
        ...
        if (!is_null($request->status)) {
            switch ($request->status){
                ...
                case('thisweek'):{                   
                    $model = $model->whereBetween(DB::raw('DATE(users.`created_at`)'),[ Carbon::now()->startOfWeek()->format('Y-m-d'),Carbon::now()->endOfWeek()->format('Y-m-d')]);
                    break;
                }

                case('thismonth'):{                   
                    $model = $model->whereBetween(DB::raw('DATE(users.`created_at`)'),[ Carbon::now()->firstOfMonth()->format('Y-m-d'),Carbon::now()->lastOfMonth()->format('Y-m-d')]);
                    break;
                }

                case('thisyear'):{                   
                    $model = $model->whereBetween(DB::raw('DATE(users.`created_at`)'),[ Carbon::now()->firstOfYear()->format('Y-m-d'),Carbon::now()->lastOfYear()->format('Y-m-d')]);
                    break;
                }               
            }
        }
        ...
        return $this->builder()
            ...
            ->parameters([
                ...
                'fnDrawCallback' => 'function( oSettings ) {
                    ...
                    $(\'#status, #clients-table_wrapper select\').select2({minimumResultsForSearch: -1});
                }',
            ])
            ...
            ->buttons(Button::make([
                ...
                'className'=>'importBtn importClientList'
                ]))
            ;
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.client') => ['data' => 'company_name', 'name' => 'client_details.company_name'],
            __('app.contactName') => ['data' => 'name', 'name' => 'name'],
