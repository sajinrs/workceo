...
class ManageAllInvoicesController extends AdminBaseController
{
    ...
    public function index(InvoicesDataTable $dataTable)
    {
        ...
        $this->paidInvoicesThisMonth = Invoice::whereBetween(DB::raw('DATE(invoices.`issue_date`)'),[ Carbon::now()->firstOfMonth()->format('Y-m-d'),Carbon::now()->lastOfMonth()->format('Y-m-d')])->where('invoices.status', '=', 'paid')->whereHas('project', function ($q) {
            $q->whereNull('deleted_at');
        })->count();
        $this->paidInvoicesThisYear = Invoice::whereBetween(DB::raw('DATE(invoices.`issue_date`)'),[ Carbon::now()->firstOfYear()->format('Y-m-d'),Carbon::now()->lastOfYear()->format('Y-m-d')])->where('invoices.status', '=', 'paid')->whereHas('project', function ($q) {
            $q->whereNull('deleted_at');
        })->count();
        $this->unpaidInvoices = Invoice::where('invoices.status', '=', 'unpaid')->whereHas('project', function ($q) {
            $q->whereNull('deleted_at');
        })->count();
        $this->unpaidInvoicesAmt = Invoice::where('invoices.status', '=', 'unpaid')->whereHas('project', function ($q) {
            $q->whereNull('deleted_at');
        })->sum('total');
        $this->partiallyPaidInvoices = Invoice::where('invoices.status', '=', 'partial')->whereHas('project', function ($q) {
            $q->whereNull('deleted_at');
        })->count();
    ...
    public function create()
    {
        ...
        $invoiceNumber = DB::table('invoices')->where('company_id', $this->user->company_id)->orderBy('id', 'desc')->first();
        $this->lastInvoice = $invoiceNumber->invoice_number + 1;
    ...
    public function store(StoreInvoice $request)
    {
        ...
        $invoiceNumber = DB::table('invoices')->where('company_id', $this->user->company_id)->orderBy('id', 'desc')->first();
        $invoice->invoice_number = $invoiceNumber->invoice_number + 1;
    ...
    public function cancelStatus(Request $request)
    {
        ...
        $invoice->status = 'cancelled'; // update status as canceled
        $invoice->save();

        return Reply::success(__('messages.invoiceUpdated'));
    }
    public function uncancelStatus(Request $request)
    {
        $invoice = Invoice::find($request->invoiceID);
        $invoice->status = 'unpaid'; // update status as uncanceled
        $invoice->save();
