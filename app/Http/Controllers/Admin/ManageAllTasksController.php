...
use App\DataTables\Admin\AllTasksArhiveDataTable;
...
class ManageAllTasksController extends AdminBaseController
{
    ...
    public function index(AllTasksDataTable $dataTable)
    {
        ...
        $this->inCompleteCount = Task::where('board_column_id', $this->taskBoardStatus[0]->id)->count();
        $this->completedCount = Task::where('board_column_id', $this->taskBoardStatus[1]->id)->count();
        $this->highPriorityCount = Task::where('priority', 'high')->count();
        $this->dueCount = Task::where('due_date', '<', Carbon::now())->where('board_column_id', $this->taskBoardStatus[0]->id)->count();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($request->has('recurring') && $request->recurring == 'yes') {
            Task::where('recurring_task_id', $id)->forceDelete();
        }
    ...
    public function archiveData(AllTasksArhiveDataTable $dataTable)
    {
        $this->projects = Project::all();
        $this->clients = User::allClients();
        $this->employees = User::allEmployees();
        ...
        return $dataTable->render('admin.tasks.archive', $this->data);
    }
