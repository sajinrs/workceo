...
class ManageTicketsController extends AdminBaseController
{
    ...
    {
        $startDate = Carbon::now()->subDays(29)->timezone($this->global->timezone);
    ...
    public function store(StoreTicket $request)
    {
        ...
        $lstTicketNumber = Ticket::orderBy('id', 'desc')->first();
        if(empty($lstTicketNumber->ticket_number)) {
            $ticketNumber = '0001';
        } else {
            $ticketNumber = str_pad($lstTicketNumber->ticket_number + 1, 4, "0", STR_PAD_LEFT); ;
        }
