...
class TimeLogReportController extends AdminBaseController
{
    ...
    public function index(TimeLogReportDataTable $dataTable) {
        ...
        $this->chartData = $this->chartData->where('project_time_logs.start_time', '>=', $this->fromDate)
            ->where('project_time_logs.start_time', '<=', $this->toDate)
            ->groupBy('date')
            ->orderBy('date', 'ASC')
            ->get([
        ...
        return $dataTable->render('admin.reports.operations.time-log', $this->data);
    }
