...
class ApiAdminManageAllTasksController extends ApiAdminBaseController
{
    use ProjectProgress;
    ...
    {
        return ApiResource::collection(Task::paginate(10));
    }

    public function show($id)
    {
        $data = Task::with('files')->findOrFail($id);
       // $data['projects'] = Project::all();
       // $data['employees'] = User::allEmployees();
       // $data['categories'] = TaskCategory::all();
      //  $data['taskBoardColumns'] = TaskboardColumn::all();
//        $completedTaskColumn = TaskboardColumn::where('slug', '!=', 'completed')->first();
//        if ($completedTaskColumn) {
//            $data['allTasks'] = Task::where('board_column_id', $completedTaskColumn->id)
//                ->where('id', '!=', $id);
//
//            if ($data['task']->project_id != '') {
//                $data['allTasks'] = $data['allTasks']->where('project_id', $data['task']->project_id);
//            }
//
//            $data['allTasks'] = $data['allTasks']->get();
//        } else {
//            $data['allTasks'] = [];
//        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $result = Task::with('files')->findOrFail($task->id);
        $response = [
            'success' => 1,
            "message" => __('messages.taskUpdatedSuccessfully'),
            'data'    => $result
        ];

        return new ApiResource($response);

        //return response()->json([ 'success' => 1, "message" => __('messages.taskUpdatedSuccessfully')]);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $response = [
            'success' => 1,
            "message" => __('messages.taskDeletedSuccessfully'),
        ];
        return new ApiResource($response);

        //return response()->json([ 'success' => 1, "message" => __('messages.taskDeletedSuccessfully')]);
    ...
    public function store(Request $request)
    {
        ...
        $response = [
            'success' => 1,
            "message" => __('messages.taskCreatedSuccessfully'),
            'data'    => $result
        ];

        return new ApiResource($response);

        //return response()->json([ 'success' => 1, "message" => __('messages.taskCreatedSuccessfully')]);
