...
class ApiAdminPaymentsController extends ApiAdminBaseController
{
    ...
    {
        return new PaymentCollection(Payment::with('project', 'invoice')->paginate(10));
    }

    public function index2(PaymentsDataTable $dataTable) {
    ...
    public function destroy($id) {
        ...
        $payment->delete();
        return response()->json([ 'success' => 1, "message" => __('messages.paymentDeleted')]);
    ...
    public function show($id){
        ...
        $data = Payment::with('project', 'invoice', 'project.client')->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            'amount' => 'required|numeric|min:1',
            'paid_on' => 'required',
            'transaction_id' => 'unique:payments,transaction_id,'.$id,
        ]);        
        ...
        //return response()->json([ 'success' => 1, "message" => __('messages.paymentSuccess')]);
    }
    ...
    {
        $projects = Project::select('project_name','id')->where('client_id', $client_id)->get();
    ...
    public function offlinePaymentSubmit(Request $request)
    {
        ...
        $offlinePayment->status            = 'approve';   
        $offlinePayment->save();

        $clientPayment = new Payment();
        $clientPayment->currency_id = $request->currency_id;
        $clientPayment->company_id = $this->company->id;
        $clientPayment->project_id = $request->project_id;
        ...
        $clientPayment->remarks  = $request->description; 
