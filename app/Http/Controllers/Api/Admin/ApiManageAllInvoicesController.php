...
class ApiManageAllInvoicesController extends ApiAdminBaseController
{
    ...
    public function index()
    {
        ...
        return new InvoiceCollection(Invoice::with('items')->paginate(10));
    }

    public function index2(InvoicesDataTable $dataTable)
    {
        $this->projects = Project::all();
        $this->clients = User::allClients();

        $this->paidInvoicesThisMonth = Invoice::whereBetween(DB::raw('DATE(invoices.`issue_date`)'),[ Carbon::now()->firstOfMonth()->format('Y-m-d'),Carbon::now()->lastOfMonth()->format('Y-m-d')])->where('invoices.status', '=', 'paid')->count();
        $this->paidInvoicesThisYear = Invoice::whereBetween(DB::raw('DATE(invoices.`issue_date`)'),[ Carbon::now()->firstOfYear()->format('Y-m-d'),Carbon::now()->lastOfYear()->format('Y-m-d')])->where('invoices.status', '=', 'paid')->count();
        $this->unpaidInvoices = Invoice::where('invoices.status', '=', 'unpaid')->count();
        $this->unpaidInvoicesAmt = Invoice::where('invoices.status', '=', 'unpaid')->sum('total');
        $this->partiallyPaidInvoices = Invoice::where('invoices.status', '=', 'partial')->count();
    ...
    public function destroy($id)
    {
        ...
        if ($firstInvoice->id == $id) {
            ...
            Invoice::destroy($id);
            return response()->json([ 'success' => 1, "message" => __('messages.invoiceDeleted')]);
        } else {
            return response()->json([ 'success' => 1, "message" => __('messages.invoiceCanNotDeleted')]);
        }
    }
    ...
    public function store(Request $request)
    {
            ...
            if (!is_null($item)) {
                InvoiceItems::create(
                    [
                        ...
                        'taxes'  => $tax[$key] ? json_encode(array($tax[0])) : '',
        ...
        $result = Invoice::findOrFail($invoice->id);

        $response = [
            'success' => 1,
            "message" => __('messages.invoiceCreated'),
            'data'    => $result
        ];

        return new ApiResource($response);

        //return response()->json([ 'success' => 1, "message" => __('messages.invoiceCreated')]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $response = [
            'success' => 1,
            "message" => __('messages.invoiceUpdated'),
            'data'    => $result
        ];

        return new ApiResource($response);

        //return response()->json([ 'success' => 1, "message" => __('messages.invoiceUpdated')]);
    }

    /*public function show($id)
    {
        $data['invoice'] = Invoice::with('items', 'project', 'client', 'currency')->findOrFail($id);
        $data['clientAddres'] = $data['invoice']->project->client->address;
        ...
        if ($data['invoice']->discount > 0) {
            if ($data['invoice']->discount_type == 'percent') {
                $data['discount'] = (($data['invoice']->discount / 100) * $data['invoice']->sub_total);
            } else {
...
//        $this->credentials = PaymentGatewayCredentials::first();
    ...
    public function addItemsProject(Request $request)
    {
        $this->items = Product::with('tax')->find($request->id);
        $this->taxes = Tax::all();
        $view = view('admin.invoices.add-item-project', $this->data)->render();
        return Reply::dataOnly(['status' => 'success', 'view' => $view]);
    }

    public function addItemsEventProject(Request $request)
    {
        $this->items = Product::with('tax')->find($request->id);
        $this->taxes = Tax::all();
        $view = view('admin.event-calendar.add-invoice-item', $this->data)->render();
        return Reply::dataOnly(['status' => 'success', 'view' => $view]);
    }
    ...
    {
        $products = Product::select('name','id')->where(['category_id' => $cid, 'allow_purchase' => 1])->get();
