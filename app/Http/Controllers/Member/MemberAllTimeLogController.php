...
use App\TaskUser;
use App\User;
use App\DataTables\Member\TimeLogDataTable;
use Carbon\Carbon;
...
class MemberAllTimeLogController extends MemberBaseController
{
    ...
    public function index(TimeLogDataTable $dataTable)
    {
        ...
        return $dataTable->render('member.time-log.index', $this->data);
    ...
    public function store(StoreTimeLog $request)
    {
        ...
        $start_date =  Carbon::parse($request->start_date)->format('d D M Y');
        $end_date =  Carbon::parse($request->end_date)->format('d D M Y');     


        $timeLog->start_time = Carbon::createFromFormat('d M Y', $request->start_date)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->global->time_format, $request->start_time)->format('H:i:s');        
        $timeLog->start_time = Carbon::createFromFormat('Y-m-d H:i:s', $timeLog->start_time, $this->global->timezone)->setTimezone('UTC');

        $timeLog->end_time = Carbon::createFromFormat('d M Y', $request->end_date)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->global->time_format, $request->end_time)->format('H:i:s');
    ...
    public function update(StoreTimeLog $request, $id)
    {
        ...
        $timeLog->start_time = Carbon::createFromFormat($this->global->date_format, $start_date)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->global->time_format, $request->start_time)->format('H:i:s');
        $timeLog->start_time = Carbon::createFromFormat('Y-m-d H:i:s', $timeLog->start_time, $this->global->timezone)->setTimezone('UTC');
        $timeLog->end_time = Carbon::createFromFormat($this->global->date_format, $end_date)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->global->time_format, $request->end_time)->format('H:i:s');
        $timeLog->end_time = Carbon::createFromFormat('Y-m-d H:i:s', $timeLog->end_time, $this->global->timezone)->setTimezone('UTC');
        $timeLog->total_hours = $timeLog->end_time->diff($timeLog->start_time)->format('%d') * 24 + $timeLog->end_time->diff($timeLog->start_time)->format('%H');
