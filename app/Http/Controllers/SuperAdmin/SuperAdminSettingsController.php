...
class SuperAdminSettingsController extends SuperAdminBaseController
{
    ...
    public function update(UpdateGlobalSettings $request, $id)
    {
        ...
        if ($request->hasFile('logo_gif_front')) {
            $setting->logo_gif_front = Files::upload($request->logo_gif_front, 'app-logo',null,null);
        }
        ...
        if ($request->hasFile('signup_background')) {
            $request->signup_background->storeAs('user-uploads', 'signup-background.jpg');
            $setting->signup_background = 'signup-background.jpg';
        }
