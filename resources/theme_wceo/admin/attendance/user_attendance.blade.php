    ...
    @if($dateData['attendance'])
        ...
        <tr>
            <td>
                ...
                <br>
                <label class="badge badge-success">{{ $currentDate->format('l') }}</label>
            </td>
        ...
        <tr>
            ...
            <td>
                @if(strtotime($currentDate)>strtotime(date('Y-m-d')))
                    <label class="badge badge-gray">Undetermined</label>

                @elseif(!$dateData['holiday'] && !$dateData['leave'])
                    <label class="badge badge-info">@lang('modules.attendance.absent')</label>
                @elseif($dateData['leave'])
                    <label class="badge badge-primary">@lang('modules.attendance.leave')</label>
                @else
                    <label class="badge badge-megna">@lang('modules.attendance.holiday')</label>
                @endif
            </td>
            <td colspan="3">
                <table width="100%" class="table-borderless">
