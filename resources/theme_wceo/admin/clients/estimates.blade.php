            ...
            <div class="row product-page-main">
                <div class="col-sm-12">
                    <ul class="showProjectTabs nav nav-tabs border-tab mb-0" id="top-tab" role="tablist">
                        ...
                        <li  class="nav-item"><a  class="nav-link active" href="{{ route('admin.clients.estimates', $client->client_details->user_id) }}"><span>@lang('app.menu.estimates')</span></a></li>
                                    ...
                                    <table class="table">
                                        ...
                                        @forelse($estimates as $key=>$estimate)
                                            <tr>
                                                ...
                                                <td>{{ $estimate->currency_symbol }}{{ currencyFormat($estimate->total) }}</td>
                                                <td>{{ $estimate->valid_till->format('m/d/Y') }}</td>
                                                <td>
                                                    @if($estimate->status == 'waiting') 
                                                    <label class="badge badge-warning">{{strtoupper($estimate->status)}}</label>
                                                    @elseif($estimate->status == 'declined') 
                                                        <label class="badge badge-danger">{{strtoupper($estimate->status)}}</label>
                                                    @else
                                                        <label class="badge badge-success">{{strtoupper($estimate->status)}}</label>
                                                    @endif                                                
                                                </td>                                            
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4">@lang('messages.noestimateFound')</td>
                                            </tr>
                                        @endforelse
                                        </tbody>
                                    </table>
