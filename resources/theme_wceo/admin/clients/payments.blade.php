            ...
            <div class="row product-page-main">
                <div class="col-sm-12">
                    <ul class="showProjectTabs nav nav-tabs border-tab mb-0" id="top-tab" role="tablist">
                        ...
                        <li  class="nav-item"><a  class="nav-link active" href="{{ route('admin.clients.payments', $client->client_details->user_id) }}"><span>@lang('app.menu.payments')</span></a></li>
                    ...
                    <div class="tab-content" id="top-tabContent">
                        <div class="tab-pane fade active show" id="top-home" role="tabpanel" aria-labelledby="top-home-tab">
                            {{--<div class="card-header">--}}
                              {{--<h5><i class="fa fa-layers"></i> @lang('app.menu.jobs')</h5>--}}
                            {{--</div>--}}
                            <div class="card-body">
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    @forelse($payments as $key=>$payment)
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ ucwords($payment->project_name) }}</td>
                                            <td>{{ $payment->invoice_id }}</td>
                                            <td>{{ $payment->currency_symbol }}{{ currencyFormat($payment->amount) }}</td>
                                            <td>{{ $payment->paid_on->format('m/d/Y ' . $global->time_format) }}</td>
                                            <td>
                                                @if ($payment->status == 'pending')
                                                    <label class="badge badge-warning">{{strtoupper($payment->status)}}</label>
                                                @else
                                                    <label class="badge badge-success">{{strtoupper($payment->status)}}</label>
                                                @endif
                                            </td>                                            
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4">@lang('messages.nopaymentFound')</td>
                                        </tr>
                                    @endforelse
                                    </tbody>
                                </table>
