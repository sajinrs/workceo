...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <section>
                <div class="sttabs tabs-style-line"> @include('member.projects.show_project_menu')  </div>
            </section>
        </div>
    </div>
    <!-- .row -->
...
&markers=color:blue%7Clabel:S%7C{{$project->client->address}}&key=AIzaSyCsLN7tz9Ww5Lt2hDS4KqaBrb8clNSwdkQ" alt="{{$project->client->address}}" border="0" width="100%">
                ...
                <div class="col-md-12">
                    ...
                    <button type="button" id="accept_action" class="btn btn-secondary pull-right m-b-20" onclick="sign();return false;"> @lang('app.sign')</button>
                </div>
                ...
                <div class="col-md-12 pr-0">                    
                    ...
                    <div class="card-header m-b-10">
                            ...
                            @if($project->isProjectAdmin || $user->can('edit_projects'))
                                <a href="javascript:;" data-toggle="tooltip" data-original-title="Update Status" id="UpdateStatus"><i class="icofont icofont-gear"></i></a>
                                <a href="{{ route('member.projects.edit', $project->id) }}" class="pull-right btn btn-secondary btn-sm">@lang('modules.jobs.jobEdit')</a></h5>                   
                            @endif
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-10 pr-0 projSteps">
                                    ...
                                    <a class="sqr {{$invoiceStatus}}" data-status="invoice" data-percentage="66" data-projstatus="awaiting pay">
                                        <i class="fas fa-file-export"></i>
                                        <h5>INVOICE</h5>
                                    </a>

                                    <a class="sqr {{$paidStatus}}" data-status="paid" data-percentage="83" data-projstatus="paid">
...
<div class="modal fade bs-modal-md in" id="projectSignModal" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
function sign() 
{
    var url = '{{ route('member.projects.sign-modal', $project->id) }}';
    $.ajaxModal('#projectSignModal', url);
}
...
$(function() {
    ...
    $('.projSteps a').click(function(){
        var status = $(this).data('status');
        if(status == 'omw' || status == 'start' || status == 'finish')
        {
            var projstatus = $(this).data('projstatus'); 
            var percentage = $(this).data('percentage');
            var token = "{{ csrf_token() }}";
            var id = '{{$project->id}}';
            var url = '{{ route('member.projects.update-job-status', ":id")}}';
            url = url.replace(':id', id);
            $.easyAjax({
                url: url,            
                type: "POST",
...
@if(!empty($invoices))
    @if($credentials->stripe_status == 'active')
        var handler = StripeCheckout.configure({
                ...
                token: function(token) {
                    ...
                    var url = "{{ route('client.stripe-public',':id') }}";
    ...
    function offlinePayment(id)
    {
        ...
        var url = '{{ route('member.project.offline-payment', ':id')}}';
        url = url.replace(':id', id);
